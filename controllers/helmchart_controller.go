...
func (r *HelmChartReconciler) fromTarballArtifact(ctx context.Context, source sourcev1.Artifact, c sourcev1.HelmChart,
	...
	if err != nil {
		return sourcev1.HelmChartNotReady(c, errorReason(err), err.Error()), err
	}
	...
	return sourcev1.HelmChartReady(c, newArtifact, cUrl, successReason(result), result.Summary()), nil
}
...
func successReason(result *chart.Build) string {
	if result.Packaged {
