...
func TestHelmRepositoryTypePredicate_Update(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "same template", old: repoA, new: repoA, want: false},
		{name: "diff template", old: repoA, new: repoB, want: true},
		{name: "new with template", old: empty, new: repoA, want: true},
		{name: "old with template", old: repoA, new: empty, want: true},
